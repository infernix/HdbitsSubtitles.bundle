...
rarfile__version__ = '2.6'

# export only interesting items
rarfile__all__ = ['is_rarfile', 'RarInfo', 'RarFile', 'RarExtFile']
...
try:
    ...
    rarfile__have_crypto = 1
except ImportError:
    rarfile_have_crypto = 0
...
class StringIOProxy(object):
    def __init__(self, stringio):
        self.rarfile_stringio = stringio
        self.rarfile_rawdata = stringio.read()
        stringio.seek(0)

    def rarfile_close(self):
        self.rarfile_stringio.seek(0)
        self.rarfile_rawdata = self.rarfile_stringio.read()
        self.rarfile_stringio.close()

    def read(self, length):
        return self.rarfile_stringio.read(length)

    def seek(self, pos, mode=0):
        return self.rarfile_stringio.seek(pos, mode)

    def tell(self):
        return self.rarfile_stringio.tell()

    def rarfile_read(self, length):
        return self.rarfile_stringio.read(length)

    def rarfile__repr__(self):
        return "<StringIOProxy: %s>" % repr(self.rarfile_stringio)

    def rarfile__getattr__(self, attr):
        if attr == 'close':
            return self.rarfile_close
        else:
            return getattr(self.rarfile_stringio, attr)

def rarfile_open(fn, *args, **kwargs):
    if isinstance(fn, StringIO.StringIO):
        return StringIOProxy(fn)
    elif isinstance(fn, StringIOProxy):
        rawdata = fn.rarfile_rawdata
...
def is_rarfile(fn):
    ...
    buf = rarfile_open(fn, "rb").read(len(RAR_ID))
    return buf == RAR_ID
...
class RarFile(object):
    ...
    def __init__(self, rarfile, mode="r", charset=None, info_callback=None, crc_check = True):
        ...
        """
        self.rarfile = rarfile if isinstance(rarfile, basestring) else rarfile_open(rarfile)
        self.comment = None
        self.rarfile_charset = charset or DEFAULT_CHARSET
        self.rarfile_info_callback = info_callback

        self.rarfile_info_list = []
        self.rarfile_info_map = {}
        self.rarfile_needs_password = False
        self.rarfile_password = None
        self.rarfile_crc_check = crc_check
        self.rarfile_vol_list = []

        self.rarfile_main = None
    ...
    def rarfile__enter__(self):
        return self

    def rarfile__exit__(self, type, value, traceback):
        self.close()

    def setpassword(self, password):
        '''Sets the password to use when extracting.'''
        self.rarfile_password = password
        if not self.rarfile_main:
            self.rarfile_parse()

    def needs_password(self):
        '''Returns True if any archive entries require password for extraction.'''
        return self.rarfile_needs_password

    def namelist(self):
        '''Return list of filenames in archive.'''
        return [f.filename for f in self.rarfile_info_list]

    def infolist(self):
        '''Return RarInfo objects for all files/directories in archive.'''
        return self.rarfile_info_list
    ...
    def getinfo(self, fname):
        ...
        try:
            return self.rarfile_info_map[fname]
        except KeyError:
            try:
                return self.rarfile_info_map[fname2]
            except KeyError:
    ...
    def open(self, fname, mode = 'r', psw = None):
        ...
        if inf.needs_password():
            psw = psw or self.rarfile_password
            if psw is None:
        ...
        if not USE_EXTRACT_HACK or not self.rarfile_main:
            use_hack = 0
        elif self.rarfile_main.flags & (RAR_MAIN_SOLID | RAR_MAIN_PASSWORD):
            use_hack = 0
        ...
        if inf.compress_type == RAR_M0 and (inf.flags & RAR_FILE_PASSWORD) == 0:
            return self.rarfile_open_clear(inf)
        elif use_hack:
            return self.rarfile_open_hack(inf, psw)
        else:
            return self.rarfile_open_unrar(self.rarfile, inf, psw)
    ...
    def printdir(self):
        ...
        for f in self.rarfile_info_list:
            print(f.filename)
    ...
    def extract(self, member, path=None, pwd=None):
        ...
        if isinstance(member, RarInfo):
            ...
            fname = member
        self.rarfile_extract([fname], path, pwd)
        ...
        if members is not None:
            for m in members:
                if isinstance(m, RarInfo):
                    ...
                    fnlist.append(m)
        self.rarfile_extract(fnlist, path, pwd)

    def testrar(self):
        """Let 'unrar' test the archive.
        """
        cmd = [UNRAR_TOOL] + list(TEST_ARGS)
        if self.rarfile_password is not None:
            cmd.append('-p' + self.rarfile_password)
        else:
            cmd.append('-p-')
    ...
    # store entry
    def rarfile_process_entry(self, item):
        if item.type == RAR_BLOCK_FILE:
            # use only first part
            if (item.flags & RAR_FILE_SPLIT_BEFORE) == 0:
                self.rarfile_info_map[item.filename] = item
                self.rarfile_info_list.append(item)
                # remember if any items require password
                if item.needs_password():
                    self.rarfile_needs_password = True
            elif len(self.rarfile_info_list) > 0:
                # final crc is in last block
                old = self.rarfile_info_list[-1]
                old.CRC = item.CRC
                old.compress_size = old.compress_size + item.compress_size
        ...
        if item.type == RAR_BLOCK_SUB and item.filename == 'CMT':
            if not NEED_COMMENTS:
                ...
                # file comment
                cmt = self.rarfile_read_comment_v3(item, self.rarfile_password)
                if len(self.rarfile_info_list) > 0:
                    old = self.rarfile_info_list[-1]
                    old.comment = cmt
            else:
                # archive comment
        ...
        if self.rarfile_info_callback:
            self.rarfile_info_callback(item)

    # read rar
    def rarfile_parse(self):
        self.rarfile_fd = None
        try:
            self.rarfile_parse_real()
        finally:
            if self.rarfile_fd:
                self.rarfile_fd.close()
                self.rarfile_fd = None

    def rarfile_parse_real(self):
        fd = rarfile_open(self.rarfile, "rb")
        self.rarfile_fd = fd
        ...
        self.rarfile_vol_list = [self.rarfile]
        while 1:
            if endarc:
                h = None    # don't read past ENDARC
            else:
                h = self.rarfile_parse_header(fd)
            if not h:
                if more_vols:
                    volume = volume + 1
                    volfile = self.rarfile_next_volname(volfile)
                    fd.close()
                    fd = rarfile_open(volfile, "rb")
                    self.rarfile_fd = fd
                    more_vols = 0
                    endarc = 0
                    self.rarfile_vol_list.append(volfile)
                    continue
                break
            h.volume = volume
            h.volume_file = volfile

            if h.type == RAR_BLOCK_MAIN and not self.rarfile_main:
                self.rarfile_main = h
    ...
    rarfile_last_aes_key = (None, None, None) # (salt, key, iv)
    def rarfile_decrypt_header(self, fd):
        if not rarfile_have_crypto:
            raise NoCrypto('Cannot parse encrypted headers - no crypto')
        salt = fd.read(8)
        if self.rarfile_last_aes_key[0] == salt:
            key, iv = self.rarfile_last_aes_key[1:]
        else:
            key, iv = rar3_s2k(self.rarfile_password, salt)
            self.rarfile_last_aes_key = (salt, key, iv)
        return HeaderDecrypt(fd, key, iv)

    # read single header
    def rarfile_parse_header(self, fd):
        try:
            # handle encrypted headers
            if self.rarfile_main and self.rarfile_main.flags & RAR_MAIN_PASSWORD:
                if not self.rarfile_password:
                    return
                fd = self.rarfile_decrypt_header(fd)

            # now read actual header
            return self.rarfile_parse_block_header(fd)
        except struct.error:
    ...
    # common header
    def rarfile_parse_block_header(self, fd):
        h = RarInfo()
        ...
        if h.type == RAR_BLOCK_MARK:
            ...
            h.header_base = h.header_base + 6
            if h.flags & RAR_MAIN_ENCRYPTVER:
                h.header_base = h.header_base + 1
            if h.flags & RAR_MAIN_COMMENT:
                self.rarfile_parse_subblocks(h, h.header_base)
                self.comment = h.comment
        elif h.type == RAR_BLOCK_FILE:
            self.rarfile_parse_file_header(h, pos)
        elif h.type == RAR_BLOCK_SUB:
            self.rarfile_parse_file_header(h, pos)
            h.header_base = h.header_size
        elif h.type == RAR_BLOCK_OLD_AUTH:
            h.header_base = h.header_base + 8
        elif h.type == RAR_BLOCK_OLD_EXTRA:
            h.header_base = h.header_base + 7
        else:
            h.header_base = h.header_size
    ...
    def rarfile_parse_file_header(self, h, pos):
        ...
        if h.flags & RAR_FILE_UNICODE:
            ...
            if u.failed:
                h.filename = self.rarfile_decode(h.orig_filename)
        else:
            h.orig_filename = name
            h.filename = self.rarfile_decode(name)
        ...
        if h.flags & RAR_FILE_EXTTIME:
            pos = self.rarfile_parse_ext_time(h, pos)
        else:
        ...
        if h.flags & RAR_FILE_COMMENT:
            self.rarfile_parse_subblocks(h, pos)
    ...
    def rarfile_parse_subblocks(self, h, pos):
        hdata = h.header_data
        while pos < len(hdata):
            ...
            if stype == RAR_BLOCK_OLD_COMMENT and pos + S_COMMENT_HDR.size <= pos_next:
                ...
                pos = pos + S_COMMENT_HDR.size
                data = hdata[pos : pos_next]
                cmt = rar_decompress(ver, meth, data, declen, sflags,
                                     crc, self.rarfile_password)
                if not self.rarfile_crc_check:
                    h.comment = self.rarfile_decode_comment(cmt)
                elif crc32(cmt) & 0xFFFF == crc:
                    h.comment = self.rarfile_decode_comment(cmt)

            pos = pos_next

    def rarfile_parse_ext_time(self, h, pos):
        data = h.header_data
        ...
        h.mtime, pos = self.rarfile_parse_xtime(flags >> 3*4, data, pos, h.date_time)
        h.ctime, pos = self.rarfile_parse_xtime(flags >> 2*4, data, pos)
        h.atime, pos = self.rarfile_parse_xtime(flags >> 1*4, data, pos)
        h.arctime, pos = self.rarfile_parse_xtime(flags >> 0*4, data, pos)
        return pos

    def rarfile_parse_xtime(self, flag, data, pos, dostime = None):
    ...
    def rarfile_next_volname(self, volfile):
        if self.rarfile_main.flags & RAR_MAIN_NEWNUMBERING:
            return self.rarfile_next_newvol(volfile)
        return self.rarfile_next_oldvol(volfile)

    # new-style next volume
    def rarfile_next_newvol(self, volfile):
        i = len(volfile) - 1
        while i >= 0:
            if volfile[i] >= '0' and volfile[i] <= '9':
                return self.rarfile_inc_volname(volfile, i)
            i = i - 1
        raise BadRarName("Cannot construct volume name: "+volfile)

    # old-style next volume
    def rarfile_next_oldvol(self, volfile):
        # rar -> r00
        if volfile[-4:].lower() == '.rar':
            return volfile[:-2] + '00'
        return self.rarfile_inc_volname(volfile, len(volfile) - 1)

    # increase digits with carry, otherwise just increment char
    def rarfile_inc_volname(self, volfile, i):
        fn = list(volfile)
        while i >= 0:
            if fn[i] != '9':
                fn[i] = chr(ord(fn[i]) + 1)
    ...
    def rarfile_open_clear(self, inf):
    ...
    def rarfile_open_hack(self, inf, psw = None):
        BSIZE = 32*1024

        size = inf.compress_size + inf.header_size
        rf = rarfile_open(inf.volume_file, "rb", 0)
        ...
        return self.rarfile_open_unrar(tmpname, inf, psw, tmpname)

    def rarfile_read_comment_v3(self, inf, psw=None):

        # read data
        rf = rarfile_open(inf.volume_file, "rb")
        ...
        return self.rarfile_decode_comment(cmt)

    # extract using unrar
    def rarfile_open_unrar(self, rarfile, inf, psw = None, tmpfile = None):
    ...
    def rarfile_decode(self, val):
        for c in TRY_ENCODINGS:
            try:
                return val.decode(c)
            except UnicodeError:
                pass
        return val.decode(self.rarfile_charset, 'replace')

    def rarfile_decode_comment(self, val):
        if UNICODE_COMMENTS:
            return self.rarfile_decode(val)
        return val

    # call unrar to extract a file
    def rarfile_extract(self, fnlist, path=None, psw=None):
...
class RarExtFile(RawIOBase):
    ...
    def __init__(self, rf, inf):
        ...
        self.rf = rf
        self.inf = inf
        self.crc_check = rf.rarfile_crc_check
        self.fd = None
        self.CRC = 0
        self.remain = 0
        self.returncode = 0
    ...
    def read(self, cnt = None):
        ...
        # actual read
        data = self.rarfile_read(cnt)
        if data:
            self.CRC = crc32(data, self.CRC)
            self.remain = self.remain - len(data)
        if len(data) != cnt:
    ...
    def seek(self, ofs, whence = 0):
        ...
        # do the actual seek
        if new_ofs >= cur_ofs:
            self.rarfile_skip(new_ofs - cur_ofs)
        else:
            # process old data ?
            #self.rarfile_skip(fsize - cur_ofs)
            # reopen and seek
            self.rarfile_open()
            self.rarfile_skip(new_ofs)
        return self.tell()

    def rarfile_skip(self, cnt):
...
class PipeReader(RarExtFile):
    ...
    def rarfile_open(self):
        RarExtFile.rarfile_open(self)

        # stop old process
        self.rarfile_close_proc()
...
class DirectReader(RarExtFile):
    ...
    def rarfile_open(self):
        ...
        self.fd = rarfile_open(self.volfile, "rb", 0)
        self.fd.seek(self.inf.header_offset, 0)
        self.cur = self.rf.rarfile_parse_header(self.fd)
    ...
    def rarfile_skip(self, cnt):
        ...
        while cnt > 0:
            # next vol needed?
            if self.cur_avail == 0:
                if not self.rarfile_open_next():
                    break

            # fd is in read pos, do the read
            if cnt > self.cur_avail:
                cnt = cnt - self.cur_avail
                self.remain = self.remain - self.cur_avail
                self.cur_avail = 0
            else:
                self.fd.seek(cnt, 1)
                self.cur_avail = self.cur_avail - cnt
                self.remain = self.remain - cnt
                cnt = 0
    ...
    def rarfile_read(self, cnt):
        ...
        while cnt > 0:
            ...
            # got some data
            cnt = cnt - len(data)
            self.cur_avail = self.cur_avail - len(data)
            buf.append(data)
    ...
    def rarfile_open_next(self):
        ...
        # open next part
        self.volfile = self.rf.rarfile_next_volname(self.volfile)
        fd = rarfile_open(self.volfile, "rb", 0)
        self.fd = fd

        # loop until first file header
        while 1:
            cur = self.rf.rarfile_parse_header(fd)
            if not cur:
    ...
    if have_memoryview:
        def readinto(self, buf):
            ...
            while got < len(buf):
                ...
                if self.crc_check:
                    self.CRC = crc32(vbuf[got : got + res], self.CRC)
                self.cur_avail = self.cur_avail - res
                self.remain = self.remain - res
                got = got + res
            return got
...
def check_returncode(p, out):
    ...
    # format message
    if out:
        msg = "%s [%d]: %s" % (exc.rarfile__doc__, p.returncode, out)
    else:
        msg = "%s [%d]" % (exc.rarfile__doc__, p.returncode)
